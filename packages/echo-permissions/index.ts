 ...
 * echo-permissions
 *
 * A cooperative permission gate for every tool call, backed by echo-core's
...
import { evaluate, readPermissions, readState } from "echo-core";
...
export default function (pi: ExtensionAPI): void {
  pi.on("tool_call", async (event, ctx: ExtensionContext) => {
    ...
    if (decision === "deny") {
      return { block: true, reason: reason ?? `Denied by echo-permissions (mode: ${state.mode})` };
    }
