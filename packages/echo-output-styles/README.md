...
Ships three built-in styles (`styles/*.md` in this package): `concise`, `explanatory`, `learning`. Project-local styles go in `.pi/echo/styles/*.md` (nearest ancestor, same resolution `echo-core` uses for permissions/state); a project-local file with the same name as a built-in overrides it.
