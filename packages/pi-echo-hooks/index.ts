...
/** Hooks from both scopes run together (additive) — unlike pi-echo-permissions' first-match-wins rules. */
...
function runHookCommand(hook: HookCommand, payload: unknown, cwd: string): Promise<HookRunResult> {
	return new Promise((resolve) => {
		...
		child.on("error", (err) => {
			clearTimeout(timer);
			resolve({ code: null, stdout, stderr: `pi-echo-hooks: failed to spawn hook command: ${err.message}`, timedOut });
		});
...
export default function (pi: ExtensionAPI): void {
	async function runFireAndForget(
		...
		for (const hook of commands) {
			...
			if (result.code !== 0 && ctx.hasUI) {
				...
				ctx.ui.notify(`pi-echo-hooks: ${eventName} hook ${reason}: ${result.stderr || result.stdout}`, "warning");
			}
		}
	}
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		for (const hook of commands) {
			...
			if (result.code === 2) {
				return { block: true, reason: result.stderr.trim() || `Blocked by pi-echo-hooks PreToolUse hook: ${hook.command}` };
			}
			if (result.code !== 0 && ctx.hasUI) {
				const reason = result.timedOut ? "timed out" : `exit ${result.code}`;
				ctx.ui.notify(`pi-echo-hooks: PreToolUse hook ${reason}: ${result.stderr || result.stdout}`, "warning");
			}
		}
		return undefined;
	});
