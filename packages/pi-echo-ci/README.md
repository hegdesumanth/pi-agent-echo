...
Exit codes: `0` success, `1` pi itself exited non-zero (forwarded as-is), `2` pi-echo-ci's own `--timeout` was hit, `3` a tool call errored and `--fail-on-tool-error` was set, `4` usage error.

Example CI invocation, with the permission gate active and locked to a non-interactive-safe mode:
```
pi-echo-ci "Fix the failing test in src/foo.test.ts" \
  --extension ./packages/pi-echo-permissions \
  --fail-on-tool-error \
  --timeout 300000
```
Since there's no human to answer an `ask` prompt in CI, set `pi-echo-permissions`' mode to something CI-appropriate (`acceptEdits` or `dontAsk`) ahead of time via `/permissions mode <mode>` in project state, or rely on the gate's own documented headless default (deny on `ask` with no UI).
