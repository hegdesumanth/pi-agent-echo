...
function printUsage(): void {
	console.error(`Usage: echo-ci <prompt> [options]
...
Exit codes:
  ...
  2  echo-ci's own --timeout was hit; the pi process was killed
...
async function main(): Promise<void> {
	...
	const exitCode = await new Promise<number>((resolve) => {
		...
		proc.on("error", (err) => {
			...
			acc.rawLines.push(`echo-ci: failed to spawn "pi": ${err.message}`);
			resolve(1);
		});
	});
	...
	if (timedOut) {
		console.error(`echo-ci: killed pi after exceeding --timeout ${opts.timeoutMs}ms`);
		process.exit(2);
	}
	if (exitCode !== 0) {
		process.exit(1);
	...
	if (opts.failOnToolError && summary.toolErrors > 0) {
		console.error(`echo-ci: --fail-on-tool-error set and ${summary.toolErrors} tool call(s) errored`);
		process.exit(3);
	}
	process.exit(0);
}
