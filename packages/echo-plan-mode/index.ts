 ...
 * echo-core's shared state.json instead of a private closure boolean, so
 * `/plan` flips the same mode echo-permissions enforces. Neither package
 ...
 * cheapest and least confusing mechanism). echo-permissions' plan-mode branch
 * in evaluate() is a deliberate backup for cases this tool-swap can't cover
 * (an in-flight call, an unknown custom/MCP tool, a state-sync bug) — not
 * redundant, and not this package's job to duplicate beyond the bash
...
import { readState, writeState } from "echo-core";
import type { Mode } from "echo-core";
