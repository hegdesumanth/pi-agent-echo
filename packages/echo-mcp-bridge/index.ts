 ...
 *
 * Design note / composition with echo-permissions: because every bridged
 ...
 * MCP tool is ALREADY subject to `echo-permissions`' generic gate with zero
 * extra wrapping — this was verified against the installed package's type
 * declarations, not assumed. What is NOT covered: `echo-core`'s
 ...
 * results do, so (unlike echo-git-status/echo-test-status) there's no
...
async function connectServer(
	...
	const client = new Client({ name: `echo-mcp-bridge/${config.name}`, version: "0.1.0" });
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		for (const config of configs) {
			...
			if ("error" in result) {
				...
				if (ctx.hasUI) {
					ctx.ui.notify(`echo-mcp-bridge: failed to connect to "${config.name}": ${result.error}`, "warning");
				}
				continue;
			}
